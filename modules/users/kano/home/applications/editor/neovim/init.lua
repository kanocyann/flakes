...
function SmartRun()
    ...
    if file_extension == 'c' then
        cmd = "cd " .. current_dir .. " && gcc % -o %:r.out -lm && ./%:r.out && rm %:r.out"
    elseif file_extension == 'cpp' then
        cmd = "cd " .. current_dir .. " && g++ % -o %:r.out && ./%:r.out && rm %:r.out"
    elseif file_extension == 'py' then
        cmd = "cd " .. current_dir .. " && python3 %"
    elseif file_extension == 'rs' then
        cmd = "cd " .. current_dir .. " && rustc % -o %:r.out && ./%:r.out"
    elseif file_extension == 'go' then
        cmd = "cd " .. current_dir .. " && go run %"
    elseif file_extension == 'kt' then
        cmd = "cd " .. current_dir .. " && kotlinc % -include-runtime -d %:r.jar && java -jar %:r.jar"
    elseif file_extension == 'java' then
        cmd = "cd " .. current_dir .. " && javac % && java %:r"
    elseif file_extension == 'rb' then
        cmd = "cd " .. current_dir .. " && ruby %"
    elseif file_extension == 'ts' then
        cmd = "cd " .. current_dir .. " && tsc % && node %:r.js"
    end

    if cmd ~= "" then
        cmd = cmd .. " && read -p 'Press Enter to continue...'"
        vim.cmd("FloatermNew --wintype=normal --height=0.4 --position=bottom sh -c " .. vim.fn.shellescape('clear && ' .. cmd))
    else
        print("Unsupported file type!")
    end
end

vim.cmd("command! -nargs=0 -bang RUN lua SmartRun()")
